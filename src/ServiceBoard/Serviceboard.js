...
function Serviceboard() {
    ...
    return (
        <>
            ...
            {sample1List.length !== 0 ?
                sample1List.map((el, key) => {
                    return (
                        <div key={key}>
                            <div className='note'>
                                <Link className='textsize' style={{ textDecoration: 'none', color: '#660000' }} to={{ pathname: `/Joblist/Serviceboardinfo?area=${el.area}&jobname=${el.jobname}&nickname=${el.nickname}&title=${el.title}` }}> {el.title}</Link>
                                <div>
            ...
            {
                nickname !== null
                    ? <div className="btn_area"><button className='loginbtn'><Link style={{ textDecoration: 'none', color: '#660000' }} to={{ pathname: `/Joblist/Servicepost?area=${area}&jobname=${jobname}` }}><AiFillEdit size='30px' /></Link></button></div>
